...
Page({
  ...
  data: {
    ...
    totalCount: 0, // 总题目数量
    UserReadingAnswersInfo: [] // 用户答题记录
  },

  onLoad: function () {
    const app = getApp();
    const newBgmUrl = './bgm/read.mp3'; // 新的背景音乐路径
  ...
  checkAnswers: function(){
    ...
    this.setData({
      ...
      UserReadingAnswersInfo: UserReadingAnswersInfo // 保存答案信息
    });
  ...
  if (userData && userData.userInfo && userData.userInfo.rank) {
    ...
    if (ranks.length > 0) {
      ...
      if (!isNaN(firstRank) && firstRank+1===selectedRank.level-1) {
        firstRank++; // 加一
        ranks[2] = firstRank.toString(); // 转换为字符串并更新到数组中
